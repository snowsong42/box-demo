...
> ✅ **已实现**（2026-06）。实际方案与本文档有差异，详见下方 [0a. 实际实现](#0a-实际实现)。
>
...
| 维度 | 本文档方案 | 实际实现 |
|------|-----------|----------|
| 识别引擎 | 百度/火山/讯飞云端 API | **本地 PC faster-whisper** (base 模型) |
| 通信协议 | HTTP REST / WebSocket | HTTP POST WAV → JSON 返回 |
| 纠错 | 无 | **DeepSeek AI 纠错**（中英双语） |
| 繁简转换 | 无 | **zhconv** 繁→简 |
| 服务器 | 云 API | **本地 Python 服务器** (`server/asr_server.py`) |

详见 [ASR语音识别.md](../guides/ASR语音识别.md)。
